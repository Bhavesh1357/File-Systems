...
int ReadFile(int fd,char *arr,int isize)
{
	...
	if(read_size<isize)
	{
		strncpy(arr,(UFDTArr[fd].ptrfiletable->ptrinode->Buffer)+(UFDTArr[fd].ptrfiletable->readoffset),read_size);  // strncpy copies the n characters of string str2 into array str1 and returns the value of str1.
	...
	while(1)
	{
		...
		else if(count==3)
		{
			...
			else if(strcmp(command[0],"read")==0)
			{
				...
				if(ret>0)
				{
					write(2,ptr,ret);   //(stdiout,data,size);
				}
				continue;
			}
			else
